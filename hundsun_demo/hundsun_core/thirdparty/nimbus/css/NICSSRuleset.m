...
#pragma mark 王金东 新增
static NSString* const kBarColorKey = @"-ios-bar-color";
static NSString* const kStatusBarStyle = @"-ios-status-bar-style";
...
- (BOOL)hasBarColor{
    return nil != [_ruleset objectForKey:kBarColorKey];
}
//-ios-bar-color
- (UIColor *)barColor{
    NIDASSERT([self hasBarColor]);
    if (_barColor == nil) {
        _barColor = [[self class] colorFromCssValues:[_ruleset objectForKey:kBarColorKey]
                               numberOfConsumedTokens:nil];
    }
    return _barColor;
}
...
- (BOOL)hasStatusBarStyle{
  return nil != [_ruleset objectForKey:kStatusBarStyle];
}
- (NSInteger)statusBarStyle{
    NIDASSERT([self hasStatusBarStyle]);
    if (_statusBarStyle == 0) {
        NSArray* values = [_ruleset objectForKey:kStatusBarStyle];
        NIDASSERT([values count] == 1);
        NSString* value = [values objectAtIndex:0];
...
#pragma mark 王金东 增加
//根据key从里面取出颜色
- (UIColor *)colorFromCssRuleForKey:(NSString*)key{
    NSArray *cssValue = [self cssRuleForKey:key];
    return [NICSSRuleset colorFromCssValues:cssValue];
}

+ (UIColor *)colorFromCssValues:(NSArray *)cssValues{
    if(cssValues != nil && cssValues.count > 0){
        return [NICSSRuleset colorFromCssValues:cssValues numberOfConsumedTokens:nil];
    }
    return nil;
}
