...
@interface HsPhotoBrowser () <HsPhotoViewDelegate, HsPhotoToolbarDelegate>
{
    // 滚动的view
...
//    CGFloat barHeight = 0;
    ...
    _toolbar = [[HsPhotoToolbar alloc] init];
...
- (void)showPhotos
{
	...
	for (HsPhotoView *photoView in _visiblePhotoViews) {
...
{
    HsPhotoView *photoView = [self dequeueReusablePhotoView];
    if (!photoView) { // 添加新的图片view
        photoView = [[HsPhotoView alloc] init];
...
{
    if (index > 0) {
        HsPhoto *photo = _photos[index - 1];
        [MKNetworkEngine downloadWithURL:photo.url];
    }
...
- (HsPhotoView *)dequeueReusablePhotoView
{
    HsPhotoView *photoView = [_reusablePhotoViews anyObject];
	if (photoView) {
