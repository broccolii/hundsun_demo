...
@interface HsRefreshHeaderView()
// 最后的更新时间
@property (nonatomic, strong) NSDate *lastUpdateTime;
@property (nonatomic, weak) UILabel *lastUpdateTimeLabel;
@end

@implementation HsRefreshHeaderView
#pragma mark - 控件初始化
/**
 *  时间标签
 */
- (UILabel *)lastUpdateTimeLabel
{
    if (!_lastUpdateTimeLabel) {
        ...
        lastUpdateTimeLabel.textColor = HsRefreshLabelTextColor;
        ...
        // 2.加载时间
        self.lastUpdateTime = [[NSUserDefaults standardUserDefaults] objectForKey:HsRefreshHeaderTimeKey];
    }
...
+ (instancetype)header
{
    return [[HsRefreshHeaderView alloc] init];
}
...
- (void)layoutSubviews
{
    ...
    CGFloat statusX = 0;
    CGFloat statusY = 0;
    CGFloat statusHeight = self.frameHeight * 0.5;
    CGFloat statusWidth = self.frameWidth;
    // 1.状态标签
...
- (void)setLastUpdateTime:(NSDate *)lastUpdateTime
{
    ...
    // 1.归档
    [[NSUserDefaults standardUserDefaults] setObject:lastUpdateTime forKey:HsRefreshHeaderTimeKey];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([HsRefreshContentOffset isEqualToString:keyPath]) {
...
- (void)setState:(HsRefreshState)state
{
    ...
    // 4.根据状态执行不同的操作
	switch (state) {
		case HsRefreshStateNormal: // 下拉可以刷新
        {
            // 设置文字
			self.statusLabel.text = HsRefreshHeaderPullToRefresh;
		...
		case HsRefreshStatePulling: // 松开可立即刷新
        {
            // 设置文字
            self.statusLabel.text = HsRefreshHeaderReleaseToRefresh;
            // 执行动画
            [UIView animateWithDuration:HsRefreshFastAnimationDuration animations:^{
		...
		case HsRefreshStateRefreshing: // 正在刷新中
        {
            // 设置文字
            self.statusLabel.text = HsRefreshHeaderRefreshing;
