...
- (void)animateView:(BOOL)isShow textField:(id)textField heightforkeyboard:(CGFloat)kheight {
	...
	if (isShow) {
		if ([textField isKindOfClass:[UITextField class]]) {
            ...
            CGPoint textPoint = [newView convertPoint:CGPointMake(0,  spacerY) toView:objectView];
			if (rect.size.height - textPoint.y < kheight)
				rect.origin.y = rect.size.height - textPoint.y - kheight + viewFrameY;
			else rect.origin.y = viewFrameY;
		}
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    if (textField.keyboardType==UIKeyboardTypeNumberPad) {
        if (numberCS==nil) {
            numberCS = [[NSCharacterSet characterSetWithCharactersInString:NUMBERS] invertedSet];
        }
        ...
        textField.text=[textField.text stringByAppendingString:[string stringByTrimmingCharactersInSet:numberCS]];
        return NO;
    }
    return YES;
}
...
- (void)addToolbarToKeyboard:(UIView *)aview{
    if ([aview isKindOfClass:[UITextField class]]) {
        ((UITextField *)aview).inputAccessoryView = keyboardToolbar;
        ((UITextField *)aview).delegate = self;
    }
